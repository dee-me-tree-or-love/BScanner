...
from todeploy.ExcelMngr import *

# the idea was to use this file as a module, but now, it's no use
def run():

    try:
        with open('dbsetup.txt', 'r') as infile: #reading is default
            loadedconfig = json.load(infile)
        #loaded setting from the file
        connection = pymysql.connect(
                host=loadedconfig['host'],
                user=loadedconfig['user'],
                password=loadedconfig['password'],
                db=loadedconfig['db'],
                charset=loadedconfig['charset'],
                cursorclass=pymysql.cursors.DictCursor,
                autocommit=True  # so that the connection commits the transactions automatically after
            )
        print("Loaded everything well")
    except:
